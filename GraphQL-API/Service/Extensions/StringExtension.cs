...
namespace Service.Extensions
{
    ...
    {
        public static string GetSQLValue(this String str)
        {
            string[] partes = str.Split(new char[] { '.' }, 2);

            return $"{partes[1]}";
        }

        public static string ToSQLWhere(this String str, string? Key = null)
        {
            string[] partes = str.Split(new char[] { '.' }, 2);

            if (partes[0].ToLower() == "in")
            {
                return $" {Key} in ({partes[1]}) ";
            }
            else if (partes[0].ToLower() == "between")
            {
                return $" {Key} between {partes[1]} ";
            }

            return $" {Key} {partes[0]} @{Key}";
